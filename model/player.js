...
playerSchema.statics.findByCredentials = async function (credential, password) {
    try {
        // Attempt to find the user by either email or mobile number
        // console.log(credential, "CREDENTIALS");
        const { email, mobile } = credential;
        let user;
...
playerSchema.methods.toJSON = function () {// defining a new instance method (toJSON) on the playerSchema.methods. Instance methods are methods that are available on
...
playerSchema.pre('save', function (next) { // This code adds a pre-save middleware to the
    // playerSchema. function will be executed before saving a user document to the database whenever the .save() method is called on a user instance.
...
playerSchema.pre('remove', function (next) {// This code adds a pre-remove middleware to the playerSchema. The middleware function will be executed before removing a user document from the database whenever the .remove() method is called on a user instance.
